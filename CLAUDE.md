...
## 第3条: 実装品質
**モックや仮のコード、ハードコードを一切禁止する。Serenaツールを使用してセマンティックなコード理解と編集を行う。新規機能の実装指示を受けたら、まずは.tmpフォルダ以下に実装計画を作成する**
...
## 第18条: 作業フォルダ管理
**全ての一時作業ファイル（バグ報告、修正計画、テスト計画等）は.tmpフォルダに作成する。tmpフォルダは使用しない。.tmpフォルダはGit管理対象外として扱う**
...
### 最新実装
- [V3.0.134 スクロールバー問題完全修正](docs/reports/v3.0.134/v3.0.134_scrollbar_bugfix_complete.md)
...
| V3.0.133 | 2025-10-27 | ❌失敗: Previewイベント+ListBoxItem判定でもスクロールバー不可 |
| V3.0.132 | 2025-10-27 | ❌失敗: IsScrollBarClick判定でもスクロールバー不可 |
| V3.0.131 | 2025-10-27 | 回転後選択維持修正・↓↑キー単一選択チェック追加・全選択後選択解除修正・Ctrl+クリック修正 |
| V3.0.130 | 2025-10-27 | ❌失敗: Dispatcher遅延実行で回転後選択が外れる（新規バグ発生） |
| V3.0.129 | 2025-10-14 | 「表示」メニュー削除・ツールバーアイコンとショートカットで全機能利用可能 |
| V3.0.128 | 2025-10-14 | UI簡素化・不要メニュー/ボタン削除（ファイル・編集・ページ・文書・テスト・自動回転）・ショートカット機能完全保持 |
