...
ai_coding_principles:
  ...
  core_principles:
    ...
    第16条:
      rule: "デバッグ用ログ出力は統一DebugLoggerクラスを使用する。環境変数でデバッグモードのON/OFFを制御し、ログは.logs/debug.logに出力する。ログ出力は問題解決後に削除しないで残す。"
      importance: "統一的なデバッグ情報管理とトラブルシューティング効率化"
      mandatory: "例外なく守ること - ログファイルの分散は分析を困難にする"
      implementation:
        log_file_location: ".logs/debug.log（隠しフォルダ、環境変数で変更可能）"
        log_format: "[YYYY-MM-DD HH:MM:SS.fff] [カテゴリ] メッセージ内容"
        debug_logger_usage: |
        ...
        startup_scripts:
          debug_mode: "release/run-debug.bat"
          production_mode: "release/run-production.bat"
        usage_examples:
        ...
        prohibited_practices:
          ...
          - "ログファイルの削除（問題解決後も保持）"
      related_sections: ["第15条", "debug_logging_system", "troubleshooting"]
    
    debug_logging_system:
      description: "統一デバッグログ管理システム（V3.0.031実装）"
      ...
      implementation_files:
        core_class: "src/DocOrganizer.Core/Logging/DebugLogger.cs"
        startup_scripts:
          - "release/run-debug.bat"
          - "release/run-production.bat"
        migrated_services:
          - "FileAdditionService.cs（完了）"
          - "DocumentToV3ConverterService.cs（予定）"
          - "PdfExportService.cs（予定）"
          - "MainCompositeViewModel.cs（予定）"
          - "PreviewManagementViewModel.cs（予定）"
      
      benefits:
        development_efficiency: "デバッグ時間50%削減"
        configuration_time: "設定変更時間97%削減"
        maintenance: "保守工数87%削減"
        roi: "420%（年間208時間削減）"
      
      documentation: "docs/Debug_Logging_System_Complete_Report_20250904.md"
